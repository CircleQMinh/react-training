...
import { UserSubmissionStatus, UserSubmissionType } from "../../utils/constant";
...
export default function PreviewPage() {
  ...
  submissions = submissions.filter(
    (q) => q.status == UserSubmissionStatus.Active
  );
  ...
  function HandlePreviewOnClick(id: any) {
    navigate(`/pages/admin/preview/${id}`);
  }

  function HandleApproveOnClick(id: any) {
    const submissionsJSON = localStorage.getItem("userSubmissions");
    if (!submissionsJSON) return;

    const submissions: UserSubmission[] = JSON.parse(submissionsJSON);

    const updatedSubmissions = submissions.map((submission) => {
      if (submission.id === id) {
        return {
          ...submission,
          status: UserSubmissionStatus.Approved,
        };
      }
      return submission;
    });
  ...
  function HandleRejectOnClick(id: any) {
    const submissionsJSON = localStorage.getItem("userSubmissions");
    if (!submissionsJSON) return;
    ...
    const updatedSubmissions = submissions.map((submission) => {
      if (submission.id === id) {
        return {
          ...submission,
          status: UserSubmissionStatus.Rejected,
        };
      }
      return submission;
    });
                ...
                <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                  {submissions?.map((sub) => (
                    ...
                    >
                      ...
                      <td className="max-w-sm p-4 overflow-hidden text-base font-normal text-gray-500 truncate xl:max-w-xs dark:text-gray-400">
                        {sub.status == UserSubmissionStatus.Active
                          ? "Active"
                          : ""}
                      </td>
                      <td className="max-w-sm p-4 overflow-hidden text-base font-normal text-gray-500 truncate xl:max-w-xs dark:text-gray-400">
                      ...
                      <td className="p-4 space-x-2 whitespace-nowrap">
                        ...
                        <button
                          onClick={() => {HandleApproveOnClick(sub.id)}}
                          type="button"
                          data-modal-target="edit-user-modal"
                          data-modal-toggle="edit-user-modal"
                          className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
                        ...
                        <button
                         onClick={() => {HandleRejectOnClick(sub.id)}}
                          type="button"
